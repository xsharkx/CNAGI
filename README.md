...
- 实现游戏机信息采集与控制
本协议将先以协议愿景为中心，聚集行业各方大神力量。将游戏机周边技术水平提升一个高度。
...
设备唯一ID|16Byte|相对厂商和产品唯一ID
P位数|1Byte|此接口控制的P位数
设备类型|2Byte|0x0001=模拟机,0x0002=彩票机,0x0003=娃娃机,0x0004=其他礼品机
...
发送参数|长度|说明
---|---|---
设备状态|2Byte|0x0000=正常，其他=故障码，详见第十章故障码
...
### **8.14 礼品机游戏结束(PID=0x030E)**

数据方向:游戏机->外接设备

说明：用于游戏机在游戏结束时通知外接设备，提供更流畅交互使用。并在结束后告诉外接设备得到的相应奖品,此指令需要与投币指令一样将以每200ms一次的频率一直发送5秒直到有回应，如5秒未回应代表离线将按超时来处理。建议游戏机将此信息保存起来下次继续执行并在游戏交互界面提示错误与重试。
...
---|---|---
事务ID|2Byte|同上事务ID
是否出奖|1Byte|0=没出礼品1=出礼品
出奖钩子|1Byte|钩子编号，0开始，单出奖口默认0

外接设备回应PID=0x0380E内容如下:
...
### **8.15 彩票机游戏结束(PID=0x030F)**

数据方向:游戏机->外接设备

说明：用于彩票机在游戏结束时通知外接设备，提供更流畅交互使用。并在结束后告诉外接设备得到的彩票,此指令需要与投币指令一样将以每200ms一次的频率一直发送5秒直到有回应，如5秒未回应代表离线将按超时来处理。建议游戏机将此信息保存起来下次继续执行并在游戏交互界面提示错误与重试。
