...
应答参数|长度|说明
---|---|---
协议版本|4Byte|当前使用的本协议版本如：112，100=V1.0，101=V1.01.需要外接设备根据此数来适配不同版本的协议，当前版本说明一般注明在文档第四章
...
说明：用于在售货机屏幕选定商品后进行支付的场景，默认超时5秒，3次重试，如果超时请发送取消购买指令，应答的二维码内容用于正扫的显示
...
说明：设备通知游戏机出货，默认超时500ms，超时重试10次。回应后应马上启动机械装置出货。此处需要掉电也能继续出货。
...
出货结果|1Byte|0=成功，1=故障，2=缺货，100=其他
...
### **11.6 更新库存(PID=0x0406)**
---

数据方向:外接设备->游戏机

说明：用户确定给设备补货后将库存同步到设备,1秒超时，超时重试10次
...
### **11.8 补货开柜(PID=0x0408)**
---

数据方向:外接设备->游戏机

说明：用于外接设备遥控游戏机批量开启某几个格子用于补货
...
---|---|---
格子数|2Byte|要打开的格子数
格子号|2Byte*格子数|要打开的格子号

外接设备回应PID=0x0488内容如下:
...
### **11.9 获取游戏正扫二维码(PID=0x0409)**
---

数据方向:游戏机->外接设备

说明：用于游戏机在屏幕上出现友好的支付提示，超时2秒
...
---|---|---|---|---|---|---
谷微动漫|0003|超级抓神|gw_whwwj|0x00030001|娃娃机|[详情](Device/0003/00030001/README.md)
蓝迪科技|0004|口红挑战|khtz01|0x00040001|口红机|[详情](Device/0004/00040001/README.md)
万代|0005|扭蛋机|bandaiegg01|0x00050001|扭蛋机|
